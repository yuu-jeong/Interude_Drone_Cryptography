...
/*ChaCha 알고리즘은  16개의 32비트 단어를 4×4 행렬로 배열 */
...
/* 상수로 선언된 초기 블록 | 16bytes 문자열 상수, 난수, context 값이 들어감 */
...
#define CHACHA20_QUARTERROUND(x, a, b, c, d) \
	...
	// XOR 연산에 사용되는 7, 8, 12, 16 상수
    x[a] += x[b]; x[d] = rotl32(x[d] ^ x[a], 16); \
    x[c] += x[d]; x[b] = rotl32(x[b] ^ x[c], 12); \
    x[a] += x[b]; x[d] = rotl32(x[d] ^ x[a], 8); \
    x[c] += x[d]; x[b] = rotl32(x[b] ^ x[c], 7);

	//10개의 루프 × 2개의 라운드/루프 = 20개의 라운드 
	for (int i = 0; i < 10; i++) 
	{
